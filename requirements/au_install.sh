...
SUPPORTED_MODELS=("openvla" "openvla-oft" "openpi" "gr00t" "gr00t_n1d6" "gr00t_n1d7" "dexbotic" "starvla" "lingbotvla" "dreamzero" "qwen3_vl" "abot_m0" "aupi")
...
install_aupi_model() {
    ...
    # 3) Install flash-attn matching whatever torch version ended up
    #    installed (aupi05/pyproject.toml pins torch==2.7.1 as of this
    #    writing, applied by openpi's own `uv sync --inexact` above, which
    #    wins over RLinf's torch pin since it runs last). Mirrors every
    #    other OpenPI-based embodied model in this script, which installs
    #    flash-attn right after the openpi package itself.
    install_flash_attn
...
main() {
    ...
    case "$TARGET" in
        embodied)
            ...
            elif [ "$MODEL" != "dreamzero" ] && [ "$MODEL" != "aupi" ]; then
