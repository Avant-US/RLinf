...
class OpenPi0ForRLActionPrediction(PI0Pytorch, BasePolicy):
    ...
    def _preprocess_observation(self, observation, *, train=True):
        """Override parent to use faithful per-sample augmentation when enabled."""
        from openpi.models_pytorch.preprocessing_pytorch import preprocess_observation_pytorch

        faithful_aug = getattr(self.config, "faithful_augmentation", False)

        if train and faithful_aug:
            ...
            augmented_images = {}
            for key in list(processed.images.keys()):
                img = processed.images[key]
                img = img / 2.0 + 0.5

                is_channels_first = img.ndim == 4 and img.shape[1] == 3
                if is_channels_first:
                    img = img.permute(0, 2, 3, 1)

                img = faithful_augment(img, is_wrist=("wrist" in key))
