...
This is the pushdoor analogue of ``tests_au/example/libero/compute_norm_stats_au.py``.
It uses the ISOLATED ``rlinf.models.embodiment.openpi_au.dataconfig`` (``pi05_pushdoor``)
and restricts the dataset to locally cached episodes to avoid HuggingFace rate-limiting
and network access, exactly like the FSDP SFT worker does at train time.

Unlike the LIBERO script (which counts local ``episode_NNNNNN.parquet`` files and
assumes they form a contiguous ``range(available_episodes)`` prefix starting at 0 --
correct for LIBERO, but WRONG for this dataset), this script reuses the fixed
``_local_episode_indices`` / ``_positional_episode_data_index`` helpers from
``rlinf.workers.sft.fsdp_vla_sft_worker_au``. Those resolve the *officially
registered* episode set from ``meta/episodes.jsonl`` (here: episodes 60..90, NOT
0..30) intersected with what's actually complete on local disk, and patch around a
lerobot indexing bug that only manifests for such non-zero-based episode ranges. See
that module's docstrings, and the "pushdoor" section of
``b/gcp/demo3/gcp_rlinf_pi05_au.md``, for the full story.
...
where ``asset_id == rlinf/pushdoor_open0622``. Point the training config's
``actor.model.model_path`` at ``{output_dir}`` so both the data loader and
``get_model`` find these stats.

Usage:
    /mnt/r/VENV/openpi_venv/bin/python tests_au/example/pushdoor/compute_norm_stats_au.py \
        --repo_id /mnt/r/DATA/SKILL/pushdoor/0622_lerobot_data \
        --output_dir tests_au/example/pushdoor/_ckpt/pi05_base_pt \
        --max_frames 34084
"""
...
def main():
    ...
    ap.add_argument("--repo_id", default="/mnt/r/DATA/SKILL/pushdoor/0622_lerobot_data")
    ap.add_argument("--output_dir", default="tests_au/example/pushdoor/_ckpt/pi05_base_pt")
    ap.add_argument("--batch_size", type=int, default=64)
    ap.add_argument("--max_frames", type=int, default=34084)
