...
#   SMOKE_MODE   1 时使用本地 tiny 数据和 eval-safe tokenizer 元数据快速验证
# =============================================================================
set -euo pipefail
...
PY
  ...
  log "SMOKE_MODE=1: preparing runtime model dir with eval-safe tokenizer EOS."
...
# Do not mutate the original model in GCS. Break only the tokenizer_config symlink
# in the local runtime directory and remap EOS to a rarely generated, valid token.
# RLinf datasets require int(tokenizer.eos_token_id), so EOS cannot be None.
# Using <|video_pad|> keeps a valid vocab id while preventing image-only eval
# generations from marking one rank finished much earlier than others. This avoids
# appending a trailing 0 attention-mask column in RLinf's manual FSDP eval loop.
