...
> **opus5的BlaBla:** 七路调研全部回来了，我把结论整合进了 `d4a_solutioin_1_c.md`（v2.1，约 2150 行）。有几条会实质性改变方案，值得单独说：

**最大的一条是我之前算错了推理预算。** 我原先把"10 Hz"换算成"每次推理 100 ms"，这是错的。在动作块 + 异步执行下，真正的约束是"推理必须在当前块耗尽前完成"——20 步的块就是 **2 秒**，宽松 20 倍。实证是 RynnWorld-4D-Policy 单次前向 1106 ms，仍然是个 9 Hz 的实时系统。这条重新划定了整个可行域，我重写了 §1.2 并把 §6.5 的推理时间轴从"100 ms 周期"改成了双时间轴（执行与推理重叠）。配套的硬性要求是状态前滚（VLASH 式），LingBot-VA 的对照显示不做这个的话 3 步任务从 85.6 掉到 32.9。

**算力该花在视频骨干而不是 VLM。** DreamZero 的同配置对照：WAM 14B 拿 50%、5B 拿 21%，而 VLA 用 8B 和 32B 的 VLM 初始化**都是 0%**。从 8B 堆到 32B 收益为零。

**存在一个与你硬件逐项同构的开源生态。** Galaxea R1-Lite 就是全向轮式基座 + 4-DoF 躯干 + 双臂 + ZED 2 头部 + 双 ZED-Mini 腕部 + T265，配 500+ 小时开源数据、开源 VLA（含 `r1pro_wbc` 配置）、开源仿真和 CoRL 2025 论文。我把它提到了主参照的位置，替代原来的双足人形线。

**双臂那部分我原来的建议只写了一半。** Co-VLA 自己的消融显示 SAE 单独用会从 63% 掉到 57%，必须配 LAC 和三个辅助损失。更关键的是 SkillVLA 的数字：在"左右臂技能各自见过、组合没见过"的任务上，π0.5 和 π0-FAST 都是 **0%**，TwinVLA 4%，带协同门控的 51%。所以我在动作头里加了一个可学习的门控 α，并把动作块改成沿运动学链自回归（base → torso → arms），这一点有 WB-VIMA、AC-DiT、Galaxea G0.5 三条独立路线佐证。

**输入侧新增了投入产出比最高的一项改造**：把观测重表达成机器人系 pointmap。See like a Robot 的阶梯消融是 27.9 → 34.7（基座系）→ 36.9（EE 系），而且注入方式有明确优劣序——逐元素相加 34.7 > 拼接 30.7 > 另开点云分支 24.2（比纯 RGB 还差）。视角随机化下纯 RGB 掉 9.6 分而 pointmap 只掉 1.8，你三路相机全动正是这个极端情形。

另外确认了两块真空白：**没有任何工作同时做"移动 + 4D"**，也**没有任何 VLA 把自身未来 swept volume 作为预测头**。前者我给了落地拆法（ego 支解析 + scene 支锚在动作块起点的基座帧，避免基座运动被建模两遍），后者在你的架构里边际成本接近零。

一条与直觉相反、影响数据采集规范的发现：**不要要求示教者两只手尽量同时动**。并发式双臂演示让 π0 从 76% 掉到 50%、Co-VLA 从 82% 掉到 56%。相关地，RoboTwin 2.0 官方双臂任务的 SMP 只有 26%——四分之三时间只有一只手在动是正常的。

新增了 §12.10（动态相机 4D 感知）和 §12.11（异步与测试时扩展）两组参考，风险表扩到 23 条，附录的设计判断扩到 22 条。可靠性方面标注了三处：Figure Helix 02 的数字全部来自公司博客无法验证，Astribot 规格来自商业评测博客建议不引用，GEN-0 的规模相变说法同样未经同行评议。
---
...
**7. 好消息：你的相机外参有一大半是解析可得的，而且已有工作证实这条路走得通。** 头部相机相对基座的位姿 = 躯干 + 颈部关节角经 FK 得到；腕部相机相对基座的位姿 = 手臂关节角经 FK 得到。**只有"基座在世界系中的位姿"需要里程计/SLAM。** [G³VLA](https://arxiv.org/abs/2606.24472) 的真机实验用的正是这个配置——"内参一次标定、外参每帧由 FK 计算"——消融显示去掉射线嵌入 −2.0、去掉 PRoPE −1.1。这是一个极强的先验，应该被用满（§3.4）。
...
在读下面的公式之前，先把全文通用的四条书写规则说清楚，后面就不再重复解释。

| 规则 | 写法 | 含义 | 例子 |
|---|---|---|---|
| **下标 = 时刻** | $x_t$ | 变量 $x$ 在时刻 $t$ 的取值 | $s_t$ 是 $t$ 时刻的本体状态 |
| **下标区间 = 时间序列** | $x_{a:b}$ | 从时刻 $a$ 到 $b$ 的整段序列 | $a_{t:t+H}$ 是从 $t$ 开始、长 $H$ 步的动作块 |
| **上标 = 语义标签** | $x^{(\text{label})}$ 或 $x^{\text{label}}$ | 标明这个量属于哪个部件/视角，**不是幂次** | $q^{\text{arm}}$ 是手臂关节角，$I^{(\text{head})}$ 是头部相机图像 |
| **位姿的方向** | $T_{A \leftarrow B}$ | 把**在 $B$ 系中表达的点**变换到 **$A$ 系**的齐次变换矩阵 | $T_{\text{base}\leftarrow\text{cam}}$ 把相机系坐标转成基座系坐标 |

> **箭头方向是最容易搞错的一点。** $T_{A \leftarrow B}$ 读作"从 B 到 A"，等价于"B 系在 A 系中的位姿"。两种读法指的是同一个矩阵，只是视角不同：把它当作变换算子时是"把 B 的坐标搬到 A"，把它当作描述时是"B 这个坐标系长在 A 的哪里"。全文统一用带箭头的写法，避免歧义。

**常用集合与算子：**

| 符号 | 名称 | 说明 |
|---|---|---|
| $\mathbb{R}^n$ | $n$ 维实向量空间 | 例：$q^{\text{arm}} \in \mathbb{R}^{14}$ 表示双臂共 14 个关节角 |
| $SE(3)$ | 三维刚体变换群 | 位置（3 维）+ 姿态（3 自由度）的组合，通常用 $4\times4$ 齐次矩阵表示。"6-DoF 位姿"就是 $SE(3)$ 中的一个元素 |
| $\pi(\cdot)$ | 相机投影 | 把相机系下的 3D 点投影成像素坐标 |
| $\pi^{-1}(u, D)$ | 反投影 | 给定像素 $u$ 与该处深度 $D$，还原出相机系下的 3D 点 |
| $\Pi_v(\cdot)$ | 视角 $v$ 的投影 | $\pi(\cdot)$ 的带视角版本：把基座系的 3D 点投到第 $v$ 路相机的图像上 |
| $\ominus$ | 位姿之差 | 两个 $SE(3)$ 元素的差异度量（$T_1 \ominus T_2 \triangleq \log(T_2^{-1} T_1)$，取李代数向量后求范数）。**不能直接用矩阵减法**，因为旋转部分不在欧氏空间里 |
| $\mathbb{1}[\,\cdot\,]$ | 指示函数 | 条件成立取 1，否则取 0。用于"只在某类样本上生效"的损失项 |
| $\text{FK}(q)$ | 前向运动学 | 由关节角算出各连杆位姿，需要 URDF。本方案大量依赖它（§4.3） |
| $\|\cdot\|$ | 范数 | 未特别标注时指 L2；损失函数中会显式写 $\|\cdot\|_1$ 表示 L1 |

#### 1.1.2 输入与输出

记：

- **观测** $o_t = \{ I_t^{(\text{head})}, I_t^{(\text{L-wrist})}, I_t^{(\text{R-wrist})} \}$，其中 $I_t^{(v)}$ 是第 $v$ 路相机在 $t$ 时刻的图像，$v \in \{\text{head}, \text{L-wrist}, \text{R-wrist}\}$。可以是单帧，也可以是长度 $L$ 的短视频片段 $o_{t-L:t}$。**三路相机全部随本体运动。**
- **相机参数** $\mathcal{C}_t = \{\,K^{(v)},\ T^{(v)}_{\text{cam}\leftarrow\text{base}}(q_t)\,\}$：
  - $K^{(v)} \in \mathbb{R}^{3\times3}$ 是第 $v$ 路相机的**内参矩阵**（焦距与主点），一次标定后固定不变；
  - $T^{(v)}_{\text{cam}\leftarrow\text{base}}(q_t) \in SE(3)$ 是**外参**，把基座系坐标变换到第 $v$ 路相机系。**注意它带 $t$ 和 $q_t$——外参随关节角变化，但可由 FK 解析计算**，不需要估计。这与"固定外参"的常规设定不同，是本方案的一个关键抓手。
- **本体状态** $s_t$，包含：
  - $q^{\text{arm}}_t \in \mathbb{R}^{2n_a}$：双臂关节角，$n_a$ 是**单臂**自由度数（7-DoF 臂则 $2n_a = 14$）；
  - $q^{\text{torso}}_t \in \mathbb{R}^{n_t}$、$q^{\text{neck}}_t \in \mathbb{R}^{n_n}$：躯干与颈部关节角，$n_t$、$n_n$ 为各自的自由度数；
  - $\dot{q}_t$：对应的关节速度（点号表示对时间求导）；
  - $T^{\text{ee,L}}_t,\ T^{\text{ee,R}}_t \in SE(3)$：左/右臂**末端执行器**（end-effector，简写 ee）的位姿，表达在基座系中；
  - $g^{\text{L}}_t, g^{\text{R}}_t \in \mathbb{R}$：双手夹爪开度；
  - $T^{\text{base}}_t \in SE(3)$：基座在 **episode 世界系**中的位姿及其速度。"episode 世界系"指以本次任务起始时刻的基座位置为原点建立的固定参照系（§3.2）。
- **语言指令** $\ell$：自然语言字符串，如"把桌上的箱子搬到货架第二层"。
- **输出 1（动作块）**：
...
$$a_{t:t+H} = \Big\{\big(q^{\text{arm,cmd}}_{t+i},\ T^{\text{ee,L/R,cmd}}_{t+i},\ g^{\text{L/R,cmd}}_{t+i},\ u^{\text{base}}_{t+i},\ q^{\text{torso/neck,cmd}}_{t+i}\big)\Big\}_{i=0}^{H-1}$$
  ...
  逐项解释：
  - 上标 $\text{cmd}$ 表示这是**命令值**（模型要求机器人达到的目标），与不带 $\text{cmd}$ 的**实测值**相区分。训练时用实测值作监督，推理时输出命令值；
  - $i$ 是块内的步索引，$i = 0$ 是最近的一步，$i = H-1$ 是最远的一步；
  - $u^{\text{base}}_{t+i}$ 是**基座命令**，本方案取**离散原语**（前进/转向/锁定等）而非连续速度，理由见 §4.1.3；
  - $H$ 是**动作块长度**（预测多少步），建议 16–50。10 Hz 下 $H=20$ 对应 2 秒的未来——这个数字在 §1.2.1 里会成为整个推理预算的基准。
- **输出 2（4D 未来）**：$\mathcal{W}_{t:t+\Delta}$，即未来 $\Delta$ 时间窗内的 4D 信息，分六个层次，见 §4.2。$\Delta$ 与 $H$ 不必相等——4D 分支通常看得比动作块更远。**表达在基座系或 episode 世界系中，不在相机系中**（原因见 §3.2）。

模型写作：

$$\pi_\theta: (o_{t-L:t},\ \mathcal{C}_t,\ s_t,\ \ell,\ \mathcal{M}_t) \ \mapsto\ (a_{t:t+H},\ \mathcal{W}_{t:t+\Delta})$$

其中 $\pi_\theta$ 是参数为 $\theta$ 的策略网络（$\pi$ 在此处是"policy"的习惯记法，与上面的投影算子 $\pi(\cdot)$ 无关，靠有无下标 $\theta$ 区分），$\mathcal{M}_t$ 是可选的持久空间记忆（§3.6），$\mapsto$ 表示"映射到"。
...
其中 $\lambda_\bullet$ 是各项的权重系数（取值见下表），$\mathcal{L}_\bullet$ 的下标标明该损失约束的对象。通用记号约定见 §1.1.1；下表中 flow matching 一项额外用到四个符号：$a^*$ 是真值动作块，$\epsilon \sim \mathcal{N}(0, I)$ 是采样的高斯噪声，$\tau \in [0,1]$ 是插值时刻（$\tau=0$ 为真值、$\tau=1$ 为纯噪声），$v_\theta$ 是网络预测的速度场，$c$ 是条件（视觉 + 状态 + 语言 + 4D latent）。
...
**Swept volume**（扫掠体积）是一个几何/运动学概念，指一个物体在一段时间内运动所**扫过的全部空间体积的并集**。在本文语境中，特指机器人全身（基座 + 躯干 + 双臂 + 头部）在未来 $H$ 个时间步的运动轨迹中占据过的所有三维空间。

用数学语言表达：

$$\text{SweptVolume} = \bigcup_{i=0}^{H-1} \text{Occupy}\big(\text{Robot},\ \text{pose}(t+i)\big)$$

即：将机器人在每个未来时刻的占据空间做并集。结果通常表示为一组**占据体素**（voxel grid）。

直觉类比：在黑暗中挥动一根发光的棒——棒划过的轨迹形成一个发光的"面"。Swept volume 是这个思想的三维推广：把机器人的整个三维几何体沿未来运动轨迹"拖过去"，所有被占据过的空间即为 swept volume。

#### 在本方案中的计算方式

本方案中 swept volume 的计算是**纯解析的**，不需要神经网络（见 §4.3）：

```
预测的动作块（关节角 + 基座命令）
  → 基座运动学积分 → 未来每帧的基座位姿 T_base(t+i)
  → 前向运动学 FK(q) → 每个连杆的 SE(3) 位姿
  → URDF 三角网格 + 连杆位姿 → 每帧的全身网格
  → 所有帧的网格沿时间做并集体素化 → swept volume
```

计算成本极低（~1–2 ms），因为只是运动学 + 几何运算，没有学习/推理开销。

#### 核心用途：安全预检与碰撞预警

Swept volume 在本方案中的主要用途是**候选动作的安全筛选**（§5.5、§6.5）：

1. 动作专家并行采样 N 个候选动作块（如 N=16~64）
2. 对每个候选，解析计算其 swept volume
3. 将 swept volume 与**场景占据栅格**（由 4D 感知分支提供）求交集
4. 交集非空 → 该候选会导致碰撞 → **拒绝**
5. 只保留安全候选，再由 value 头打分选最优

这一步在推理时间轴上对应 §6.5 的 `t=250ms` 阶段（"FK + swept volume + 自碰撞 + 场景占据求交，~20 ms，纯几何"）。

#### 具体例子

假设机器人的右臂要从桌面左侧移到右侧抓杯子：

- **单帧占据**：只知道右臂"此刻在哪"——无法预判中间是否会撞到障碍
- **Swept volume**：知道右臂从起点到终点**整条弧线扫过的全部空间**——如果弧线经过桌上的花瓶，swept volume 与花瓶占据就有交集 → 碰撞预警 → 换一条绕过花瓶的轨迹

对双臂移动基座机器人尤其关键：基座在移动、躯干在转、双臂在同时动作，整个机器人扫过的空间非常复杂，纯靠直觉无法判断安全性。

#### 本文各处出现的汇总

| 出处 | 上下文 | 要点 |
|---|---|---|
| §0 TL;DR 结论 8 | "…能解析地、零误差地得到…swept volume…**这部分 4D 完全不需要神经网络去猜**" | 强调 swept volume 是动作预测的**免费副产品**，边际成本接近零 |
| §0 TL;DR 结论 8 | "目前没有任何 VLA 把自身未来 swept volume 作为预测头输出" | 标记为**文献空白**——现有 swept volume 工作（如 NeuralSVCD）都是独立于策略的碰撞检测模块，不在策略回路内 |
| §2.9（opus 前言） | "没有任何 VLA 把自身未来 swept volume 作为预测头…后者在你的架构里边际成本接近零" | 同上，两轮独立调研均确认为空白 |
| §4.2 本体专属 4D 表格 | "全身 swept volume：未来 H 步内整个机器人扫过的占据体素；解析：FK + URDF 网格沿时间扫描；用途：安全预检，与场景占据求交即得碰撞预警" | swept volume 的**正式定义与计算方法** |
| §4.2 空白说明 | "第一项在文献里是空白…现有 swept volume 工作都是独立于策略的碰撞检测模块，不在策略回路里…边际成本接近零" | 再次强调这是本方案可直接获取的**结构性红利** |
| §4.3 解析捷径流程图 | `POSE → SWEPT["swept volume 占据"]` → `SWEPT → SAFE["安全预检 / 碰撞查询"]` | swept volume 在解析 4D 产物链中的位置 |
| §4.3 本体 4D 产物表 | "全身未来扫过的 swept volume → 网格沿时间扫描 → 安全预检的核心产物" | 与自碰撞距离并列为**双臂移动本体的关键安全量** |
| §4.3 末尾空白确认 | "没有任何 VLA 把自身未来 swept volume 作为输出…没有任何工作把 URDF 自渲染深度当作免费度量锚点" | 与解析渲染锚点一起构成本方案两个**核心创新点** |
| §5.5 技术清单 T16 | "解析安全检查 + 拒绝采样：采样多个候选动作块，用解析 swept volume 与自碰撞检查过滤，几乎零成本消除一类硬失败" | swept volume 在**测试时扩展**策略中的具体角色 |
| §6.1 架构图 | "swept volume / 自碰撞距离"列于"全身解析 4D"模块内 | 在整体架构中的位置——解析层的输出之一 |
| §6.1 连接关系 4 | "解析 4D → 安全门控：swept volume 与自碰撞距离直接用于候选动作筛选" | swept volume 是**解析 4D 与动作选择之间的桥梁** |
| §6.5 推理时间轴 | `t=250ms ├─ 对 N 个候选：FK + swept volume + 自碰撞 + 场景占据求交 (~20 ms, 纯几何)` | 实际推理中的**时序位置与耗时** |
| §7 需求-方案映射表 | "手臂/全身在 4D 空间怎么活动 → …swept volume…" | 映射到用户需求"4D 信息越多越好" |
| §9 工程架构图 | "FK / 渲染 / swept volume / 自碰撞"列于解析层 | 在工程部署架构中的位置 |
| §11 能力分级 F1 | "F1 解析层：…swept volume" | swept volume 属于**最底层的基础设施能力**，是后续所有能力的前提 |
| 附录 A 判断 A12 | "测试时解析安全检查是必做项…本体 4D 反正要算，检查几乎零成本" | swept volume 检查是**不可省略的设计判断** |

#### 与现有工作的关系

现有文献中的 swept volume 工作（如 **NeuralSVCD**）都是**独立的碰撞检测模块**：给定一条已规划好的轨迹，事后检查是否碰撞。它们不在策略回路内，不随动作块一起产生，也不参与动作选择。

本方案的创新在于：**把 swept volume 直接嵌入策略回路**——模型预测动作 → 解析计算 swept volume → 用它筛选候选 → 选出安全且最优的动作。由于动作本来就要预测、FK 本来就要算，swept volume 只是在已有计算链上多做一步体素化并集，**边际成本接近零**。这是本方案反复强调的"结构性红利"。

## 二. SAE 与 LAC（Co-VLA 的双臂动作头核心组件）

SAE（Structured Action Expert，结构化动作专家）和 LAC（Latent-Aware Controller，潜变量感知控制器）均来自 [Co-VLA](https://arxiv.org/html/2606.20285v1)，是专门为双臂机器人设计的动作头架构。本方案在 §4.1.2 将其作为双臂动作头的首选结构，并在 §6.2 的选型表中明确要求"SAE + 协同门控 α + LAC，三者必须一起"。

#### 二.1 SAE — Structured Action Expert（结构化动作专家）

**问题背景**：常规做法是把双臂动作表示为一个拼接的大向量 $[a_L;\ a_R]$，由同一个网络直接回归。Co-VLA 指出，这**混淆了两种本质不同的变异来源**（§4.1.2）：

| 变异来源 | 性质 | 例子 |
|---|---|---|
| 任务级协同意图 | 低频、结构化、跨任务可复用 | "两手一起抬"这个模式可迁移到不同物体 |
| 每臂执行细节 | 高频、本体相关 | 左手抓在锅把哪个位置、手指张开角度 |

**SAE 的做法**是把动作做**加性分解**：

$$a_L = \alpha \cdot W_L(z_{\text{shared}}) + z_L, \quad a_R = \alpha \cdot W_R(z_{\text{shared}}) + z_R$$

各符号含义：

| 符号 | 名称 | 含义 |
|---|---|---|
| $z_{\text{shared}}$ | shared coordination latent | **任务级协同意图**——"两只手要怎么配合"。低频、结构化、跨任务可复用 |
| $z_L$、$z_R$ | residual latent | 每只手臂各自的**执行微调**。高频、本体相关 |
| $W_L$、$W_R$ | 线性投影 | 把共享 latent 解码成对应手臂的动作分量 |
| $\alpha$ | 协同门控 | 控制耦合强度。$\alpha \to 0$ 时两臂完全独立（退化为两个单臂策略），$\alpha \to 1$ 时进入紧耦合模式。**注意：$\alpha$ 是 SkillVLA 补充的，Co-VLA 原文没有这个门控** |
...
- 真机 OOD 场景从 13% → 27%
- 完成时间缩短 **25%**
- 失败可归因——是协同计划错了（$z_{\text{shared}}$ 的问题），还是单臂执行错了（$z_L / z_R$ 的问题）

#### 二.2 为什么 SAE 单独用会掉分（63% → 57%）

这是本文反复引用的一条关键消融结论（§4.1.2、§2.8 设计要求 1、附录 A 判断 A2、风险 R19）。

原因：结构化分解把动作空间**切开了**——左臂和右臂各走各的表征通道。如果没有额外机制把它们重新"绑住"：

1. **信息分配失控**：$z_{\text{shared}}$ 本该捕捉协同意图，但训练中残差 $z_L / z_R$ 可能把协同信息也吸收掉（因为残差的容量足够大），导致 $z_{\text{shared}}$ 退化成空壳
2. **两臂失去协同**：没有同步机制时，两臂在时间轴上各走各的节奏，紧耦合任务（如双手抬同一刚体）直接失败
3. **平滑出错**：动作块拼接时的 EMA 平滑如果不区分"协同段"和"独立段"，会在独立段过度约束、在协同段约束不足

这就是为什么必须配合 LAC 和辅助损失。

#### 二.3 LAC — Latent-Aware Controller（潜变量感知控制器）

LAC 是**部署时**工作在关节命令层的控制器。它**读取 SAE 产生的 $z_{\text{shared}}$ 和 $z_L / z_R$ 的能量比**，在线动态调节三件事（§4.1.2）：

| 调节项 | 含义 | 调节逻辑 |
|---|---|---|
| **同步强度** | 两臂的时间同步有多紧 | $z_{\text{shared}}$ 能量高（紧耦合）→ 收紧同步；能量低（独立）→ 放松 |
| **平滑度** | 动作块拼接时的平滑程度 | 协同段收紧平滑以保持双臂一致，独立段放松以保留各臂灵活性 |
| **安全裕度** | 速度上限等安全参数 | 协同段限速（两手同时作用于物体时更需谨慎），独立段正常 |

LAC 的关键特性：

- **不需要力控或阻抗控制**，纯运动学层面工作，与本方案"不考虑力控"的约束完全兼容
- **"latent 感知"是核心**：Co-VLA 报告朴素 EMA 平滑（不看 latent，始终用固定权重平滑）也会掉分。只有让平滑/同步的强度由 $z_{\text{shared}}$ 的实际能量驱动，才能正确处理"协同段紧同步、独立段松同步"的切换

#### 二.4 三个辅助损失

除了 SAE 和 LAC，还必须配三个**任务自适应的辅助损失**（§4.1.2），它们共同确保信息分配正确：

| 损失 | 作用 | 为什么需要 |
|---|---|---|
| $\mathcal{L}_{\text{sparse}}$（残差稀疏性） | 逼迫协同信息进入 $z_{\text{shared}}$，不被残差吸收 | 没有它，残差容量足够大，会把 shared 该学的东西"偷走" |
| $\mathcal{L}_{\text{shared}}$（共享一致性） | 约束 $z_{\text{shared}}$ 确实编码了跨臂共享的语义 | 保证 shared latent 不退化 |
| $\mathcal{L}_{\text{sync}}$（时间同步性） | 约束紧耦合段两臂的时间对齐 | 双手抬同一刚体时必须同步发力/移动 |

"任务自适应"指这三个损失的权重**随协同模式 `coord_mode` 变化**——独立并行任务时 $\mathcal{L}_{\text{sync}}$ 权重应当接近 0（强制同步反而有害），紧耦合任务时拉高。
...
| 出处 | 上下文 | 要点 |
|---|---|---|
| §0 TL;DR 结论 10 | "SAE 单独用会掉分（63% → 57%），必须配 LAC 和三个辅助损失" | 首次提出"不可拆"的结论 |
| §2.8 Co-VLA 表格 | "SAE…配 LAC 在部署时按 shared/residual 能量比在线调节同步强度、平滑度与安全裕度" | SAE 和 LAC 的正式定义与实测数字 |
| §2.8 设计要求 1 | "动作头必须结构化，但结构化不能单独用…只抄一半会比不抄更差" | 提炼为设计规则 |
| §4.1.2 详述 | SAE 的加性分解公式、LAC 的工作机制、三条配套要求的详细说明 | 本文对 SAE/LAC 最完整的技术描述 |
| §6.2 选型表 | "Co-VLA 式 SAE + SkillVLA 式协同门控 α + LAC（三者必须一起）" | 选型决策 |
| §9 参考实现表 | "Co-VLA：SAE + LAC 需自行实现。注意 SAE 必须与 LAC、三个辅助损失一起用" | 工程实现提醒 |
| 风险 R19 | "只抄了 Co-VLA 的一半（有 SAE 无 LAC/辅助损失），结果比不抄还差" | 列为中等风险 |
| 附录 A 判断 A2 | "双臂动作头用共享+残差分解，且必须配协同门控与辅助损失" | 设计判断速查 |

---
## 三. 不做异步带来的影响
可以不做异步，而且我建议就从同步开始。先纠正一件我在文档里没说清楚、容易造成误导的事。

### 我引用的那组数字不是"异步 vs 同步"

§1.2.1 里的 LingBot-VA 对照（90.4 vs 74.3，3 步任务 85.6 vs 32.9）比的是**异步做对 vs 异步做错**。同步执行是第三条路，它根本不存在那个失败模式——模型看到的观测就是执行起点的观测，没有错位可言。所以那组数字不构成"必须做异步"的理由，它只说明"做了异步就必须配状态前滚"。

### 对成功率的影响：证据说基本没有

三条比较硬的依据：

[RTC 论文](https://proceedings.neurips.cc/paper_files/paper/2025/file/300ccb2187dedd4edcc07f7e76d8e553-Paper-Conference.pdf)（NeurIPS 2025，Physical Intelligence）自己把同步列为基线，并注明它是"**先前工作的默认推理策略**"。他们在 6 个真机双臂任务上用 π0.5 做的对照，结论原话是"**尽管同步推理往往达到相近的最终分数**，RTC 更早完成任务"。也就是说差距主要在速度而非成功与否。

[异步方法系统性对照](https://arxiv.org/html/2605.08168)那篇更直接：他们把 $d=0$ 明确标注为 "Synchronous Execution"，并说"所有变体在 $d=0$ 应当匹配基础策略"。**同步是各种异步方法努力追赶的上界，不是下界。**

还有一条你可能没想到的：所有我在文档里引用的成功率数字——LIBERO 98.5%、RoboTwin 2.0 的 92.9%、π0.5 的各项成绩——**都是同步跑出来的**。仿真基准里策略推理时世界是冻结的。所以"同步能达到什么水平"这个问题，答案就是文档里那一整张成绩表。

### 但有三类情况同步确实会掉成功率

**世界会自己动的任务。** 人机交接、传送带、场景里有别人在走动。仿真里同步是免费的（世界跟着冻结），真机上不是——机器人停了，世界没停。你的任务如果基本是准静态的（取放、开门、搬运），这条不成立。

**精密且不允许重试的任务。** RTC 的 light candle 任务是唯一一个不允许重试的，也是同步唯一大幅落后的任务。允许重试的任务里 π0.5 会自己"失败→重来"，把同步的劣势磨平了。

**暂停本身带来的分布偏移。** 这条最容易被忽略，而且 RTC 有直接证据：他们发现**即使把暂停时间从计时里扣掉，RTC 仍然比同步完成得快**。机制是示教数据全是连续运动采集的，运动中途停住对策略是 OOD；物理上还有静摩擦大于动摩擦、物体在暂停期间沉降或滑动、动量不连续等问题。

### 除成功率外的代价

**吞吐量线性下降。** RTC 实测同步的吞吐随推理延迟线性退化。按你的配置估算：块长 2 秒、重型架构单次推理 1 秒左右，每个周期多 50% 时间，长程任务走 20 个块就是多 20 秒。

**反应性变差。** 同步下你必须执行完整块才能重规划。0.3 秒时出了岔子，要等到 2.0 秒才发现。异步配 50% 阈值的话重规划间隔是 1 秒。**注意这一条可以单独解决**：缩短块长或只执行块的前半段再暂停，代价是暂停更频繁。

**工程上反而省很多。** 不需要 gRPC 的 PolicyServer/RobotClient、不需要 VLASH 状态前滚、不需要 RTC 的 inpainting 引导、不需要 chunk 融合。R17 那条风险直接消失。调试也确定性得多。

### 一个针对你的设计几乎免费的缓解

分布偏移那条可以用你已有的设计消掉大半：**在自然停顿点暂停，而不是在运动中途暂停**。你的动作块里本来就输出 `phase` 标签（移动段/操作段/过渡段），基座又用的是"走 30 cm 然后干净停住"这类离散原语——这些原语天然有停顿点。规则可以很简单：基座原语执行完之后允许暂停，抓取动作进行到一半、插拔对孔过程中不允许暂停（宁可多执行一段旧块）。这把"暂停"从随机时刻挪到了示教数据里本来就存在停顿的时刻。

### 一个对你特别有利的正面效应

同步下推理预算**没有上限**——想多久就多久，代价只是暂停更长。这跟你"算力不限、频率不是瓶颈"的前提配合得比异步还好。异步的预算被块长卡死在 2 秒，同步下你可以跑 best-of-64 加完整的世界模型 rollout 打分，花 5 秒也无所谓。§5.5 的测试时扩展在同步下几乎是白拿的。

### 什么时候必须转异步

一个反向的证据值得留意：RTC 的真机实验里，朴素异步（temporal ensembling）在注入 +100 ms 和 +200 ms 延迟时**根本跑不起来**——振荡大到触发机器人的保护性急停，而同步只是线性变慢但仍能跑完。**所以顺序是：做对的异步 > 同步 > 朴素异步。** 你的架构延迟高，如果不打算认真做异步，同步是明显更安全的选择，绝不要走中间那条。

真要转的时候，那篇系统性对照给了选型建议：长块（H=50）场景 **A2C2** 最可靠（一个每控制步跑的轻量残差修正头，不动主策略，可与其他方法叠加）；**VLASH** 的前提是"有准确的前向状态模型"——你有 URDF FK 加已知基座运动学，这个前提对你恰好成立，是少数天然适配的场景。

## 四. Ray Map（射线图）

### 基本概念

**Ray map**（射线图）是一种将相机的几何信息编码进视觉 token 的表征方式。对图像中的每个像素（或 patch），ray map 记录的是**该像素对应的三维投影射线**——即从相机光心出发、穿过该像素所指向的三维空间方向。

直觉理解：一张 RGB 图像告诉你"每个像素看到了什么颜色"，而 ray map 告诉你"每个像素对应的光线是从哪里出发、朝哪个方向射出去的"。两者结合，模型就同时知道了**内容**（RGB）和**几何**（这个内容在三维空间的哪条射线上）。

数学上，对视角 $v$ 的像素 $u = (u_x, u_y)$：

$$\mathbf{r}_{\text{cam}} = \text{normalize}\big(K^{-1} \cdot [u_x,\ u_y,\ 1]^\top\big)$$

其中 $K$ 是相机内参矩阵。这给出了**相机坐标系**下的射线方向。要让不同视角的射线可以比较，需要统一到同一个坐标系（本方案选择**基座系**）：

$$\mathbf{r}_{\text{base}} = R_{\text{base} \leftarrow \text{cam}} \cdot \mathbf{r}_{\text{cam}}$$

再加上光心在基座系中的位置 $\mathbf{o}_{\text{base}} = T_{\text{base} \leftarrow \text{cam}}[:3, 3]$，一条射线就由 $(\mathbf{o}_{\text{base}},\ \mathbf{r}_{\text{base}})$ 这 6 个数完全确定。

将整张图像所有像素（或所有 patch 中心）的射线排列起来，就构成了一张 ray map——形状为 $[H/p,\ W/p,\ 6]$（$p$ 是 patch 大小），其中每个位置存 3 维射线方向 + 3 维光心坐标。

### 与 Plücker 射线的关系

文档 §3.2.1 的消融表（[See like a Robot](https://arxiv.org/abs/2607.11498)）中提到了"Plücker 射线"，它是 ray map 的一种数学等价表示。**Plücker 坐标**用 $(\mathbf{d},\ \mathbf{m})$ 表示一条空间直线：

- $\mathbf{d}$：射线方向（3 维）
- $\mathbf{m} = \mathbf{o} \times \mathbf{d}$：力矩向量（3 维），编码了射线距原点的位置信息

两种表示数学上等价，都是 6 维，都能唯一确定一条空间射线。在 See like a Robot 的消融中，Plücker 射线的收益是 +0.8（27.9 → 28.7），远小于 pointmap 的 +6.8（27.9 → 34.7）。**这不意味着射线信息没用，而是说"射线方向"不如"射线方向 + 深度 = 具体 3D 坐标"信息量大。** 两者是互补关系：射线告诉模型"这个像素朝哪看"，pointmap 进一步告诉模型"这个像素看到的东西在哪"。

### 为什么在本方案中特别重要

在常规设定（相机固定在桌面上）中，ray map 的值相对有限——因为相机不动，模型可以靠"记住固定的空间布局"来蒙混过关，每个像素对应的射线是固定的，相当于一个可被隐式记忆的常量。

**但在本方案中，三路相机全部在动**（§1.3），ray map 的价值被大幅放大：

1. **头部相机**随基座平移 + 头部转动，射线方向每帧都在变
2. **腕部相机**随手臂高速运动，射线方向剧烈变化
3. 模型不可能靠记忆来推断"这个像素对应三维空间的哪里"——**必须每帧实时计算并显式告知**

正因如此，文档将"逐帧 ray map"列为**必做项**（§3.3 派生信息表第 1 条，五星优先级），并在 §3.4 将其作为几何注入的首选方案。

### 在本方案中的计算方式

Ray map 的计算是**纯解析的**，不需要任何学习（§3.3、§3.4）：

```python
# 伪代码：逐帧 ray map 计算（§3.4 (a)）
for v in ["head", "wrist_L", "wrist_R"]:
    # 外参由 FK 解析得到——每帧重算
    T_base_cam = forward_kinematics(urdf, q_t, camera_link[v])
    # 内参固定（一次标定）
    ray_cam    = normalize(inv(K[v]) @ pixel_grid)            # [H/p, W/p, 3]
    # 转到基座系
    ray_base   = T_base_cam[:3, :3] @ ray_cam                 # 射线方向
    origin     = T_base_cam[:3, 3].broadcast_to(ray_base.shape)# 光心位置
    # → ray_map[v] = concat([ray_base, origin])               # [H/p, W/p, 6]
```

关键特性：

- **外参来自 FK**，精确已知（§3.4 引用 G³VLA 原话："内参一次标定、外参每帧由前向运动学计算"）
- 计算代价 ~0 ms（纯矩阵乘法）
- 每帧必须重算（因为相机在动）

### 如何注入模型

Ray map 计算出来后，需要注入到视觉编码器的输出中。本方案采用 [G³VLA](https://arxiv.org/html/2606.24472) 的做法（§3.4 (a)）：

1. 将 ray map 通过一个**零初始化的 MLP** 投影成与视觉 token 相同维度的嵌入
2. **逐元素相加**到视觉编码器（SigLIP / DINOv2）输出的 token 上
3. 在注意力层中配合 **PRoPE**（投影位置编码），让不同视角的 token 之间带上相对相机几何

```python
geo_emb    = zero_init_mlp(concat([ray_base, origin]))   # 零初始化 MLP
tokens[v]  = vision_encoder(img[v]) + geo_emb             # 逐元素相加
tokens      = cross_view_attn_with_PRoPE(tokens, relative_poses)
```

**零初始化**是关键设计：初始时 MLP 输出全零，不破坏预训练权重（与 §3.2.1 中 pointmap 的"逐元素相加"、ControlNet 的零卷积同源）。训练过程中模型逐渐学会利用几何信息。
...
### Ray map vs Pointmap vs Ego3D PE：三者的关系

文档中频繁出现三种几何注入方式，它们各自编码的信息不同，是**互补**关系（§3.4 (e)）：

| 方式 | 编码的信息 | 是否需要深度 | 来源 |
|---|---|---|---|
| **Ray map** | "这个像素对应哪条射线"（方向 + 光心） | **否**，只需内参 + 外参 | G³VLA |
| **Pointmap** | "这个像素看到的东西在 3D 空间的哪个点" | **是** | See like a Robot |
| **Ego3D PE** | "这个 patch 对应的 3D 坐标"（正弦编码） | **是** | SpatialVLA |

三者的关系可以这样理解：
- **Ray map** = 射线方向（已知）——告诉模型"从哪看、朝哪看"
- **Pointmap** = 射线方向 + 深度（需要深度传感器或估计）——告诉模型"看到的东西在哪"
- **Ego3D PE** = 与 pointmap 类似，但用正弦位置编码而非逐元素相加

本方案的建议组合（§3.4 (e)）：**ray map + Ego3D PE + pointmap 作为主路**，三者叠加使用。

### 本文各处出现的汇总

| 出处 | 上下文 | 要点 |
|---|---|---|
| §0 推荐架构（一句话） | "输入侧换成基座系/EE 系 pointmap + **解析外参射线嵌入**" | ray map 是推荐架构的输入侧核心组件之一 |
| §2 领域全景地图 | "多相机几何注入：**ray map** / PRoPE / Ego3D PE" | 列为输入侧三大技术之一 |
| §2.4 G³VLA 条目 | "内参条件 **ray embedding** + PRoPE 投影位置编码 + 双向跨视角融合" | ray map 的主要来源论文 |
| §3.2.1 See like a Robot 消融表 | "+ Plücker 射线：28.7（+0.8）" | Plücker 射线（ray map 的等价形式）单独使用的收益量化 |
| §3.3 派生信息表第 1 条 | "三路相机 **ray map（基座系）**：解析，FK + 内参；代价 ~0；★★★★★ 必做" | 列为**五星优先级、必做项** |
| §3.4 (a) 详述 | ray map 的完整计算伪代码、零初始化注入方式、G³VLA 消融数字（去掉射线嵌入 −2.0） | 本文对 ray map **最完整的技术描述** |
| §5 架构输入框图 | "ray map / PRoPE"列于输入模块 | 在架构图中的位置 |
| §6.1 详细数据流 | "逐帧 ray map（FK 解析，基座系）" | 在推荐方案数据流中的位置 |
| §6.1 视觉/几何编码模块 | "零初始化 **ray embedding**（G³VLA，逐帧重算）" | 编码器内部的注入位置 |
| §6.2 选型表"几何注入"行 | "逐帧 ray map + PRoPE（零初始化）+ Ego3D PE" | 选型决策：三者叠加 |
| §6.5 推理时间轴 | "解析计算：pointmap、**ray map**、ego-flow、本体掩膜…(~5 ms)" | 实际推理中的时序与耗时 |
| §11 能力分级 F4 | "F4 几何注入：逐帧 **ray map** · PRoPE · Ego3D PE" | 属于第四级能力（依赖 F0–F2） |
| §5.5 技术清单 T4 | "零初始化几何注入：ray embedding 用零初始化投影层加到视觉 token 上" | 实现技巧 |

## 五. Plücker Ray（Plücker 射线 / 普吕克射线）

### 问题背景：怎么用数字表示一条空间直线？

三维空间中的一条直线（射线）有 4 个自由度（过一个点、朝一个方向，去掉方向的尺度自由度）。但直接用"一个点 + 一个方向"来表示有个问题：**同一条线可以选无穷多个不同的点**，表示不唯一。

**Plücker 坐标**是 19 世纪数学家 Julius Plücker 提出的一种用 **6 个数**唯一表示空间直线的方法（在齐次意义下），具有良好的代数性质。

### 定义

给定一条射线，已知其：
- **方向向量** $\mathbf{d} \in \mathbb{R}^3$（单位化或非零）
- **线上任意一点** $\mathbf{o} \in \mathbb{R}^3$（比如相机光心）

Plücker 坐标为一个 6 维向量 $(\mathbf{d},\ \mathbf{m})$：

$$\mathbf{d} = \text{射线方向}, \quad \mathbf{m} = \mathbf{o} \times \mathbf{d}$$

其中 $\mathbf{m}$ 称为**力矩向量**（moment）。

### 为什么 $\mathbf{m}$ 能编码位置

叉积 $\mathbf{o} \times \mathbf{d}$ 的几何意义：

- **方向**：垂直于 $\mathbf{o}$ 和 $\mathbf{d}$ 所在的平面
- **大小**：$|\mathbf{m}| = |\mathbf{o}| \cdot |\mathbf{d}| \cdot \sin\theta$ = 射线到坐标原点的**距离** $\times |\mathbf{d}|$

所以 $\mathbf{m}$ 编码了"这条线离原点有多远、在哪个方位"。

**表示唯一性的关键**：选线上不同的点 $\mathbf{o}$，算出的 $\mathbf{m}$ 相同。证明：换一个点 $\mathbf{o}' = \mathbf{o} + t\mathbf{d}$，则

$$\mathbf{o}' \times \mathbf{d} = (\mathbf{o} + t\mathbf{d}) \times \mathbf{d} = \mathbf{o} \times \mathbf{d} + t(\mathbf{d} \times \mathbf{d}) = \mathbf{m} + 0 = \mathbf{m}$$

无论选线上哪个点，力矩向量 $\mathbf{m}$ 不变——这就消除了"选点不唯一"的歧义。

### 具体例子

一条从相机光心 $\mathbf{o} = (1, 0, 0)$ 出发、朝 $\mathbf{d} = (0, 0, 1)$ 方向的射线：

$$\mathbf{m} = \mathbf{o} \times \mathbf{d} = (1,0,0) \times (0,0,1) = (0, -1, 0)$$

Plücker 坐标 = $(\mathbf{d}, \mathbf{m}) = (0, 0, 1, 0, -1, 0)$。

验证唯一性：换同一条线上的另一个点 $\mathbf{o}' = (1, 0, 5)$：

$$\mathbf{m}' = (1,0,5) \times (0,0,1) = (0 \cdot 1 - 5 \cdot 0,\ 5 \cdot 0 - 1 \cdot 1,\ 1 \cdot 0 - 0 \cdot 0) = (0,-1,0) = \mathbf{m}$$

结果不变，确认同一条线的 Plücker 坐标唯一。

### 在视觉/机器人中的用法

在本文档的语境中（§3.2.1 See like a Robot 的消融表），"Plücker 射线"指的是：**对图像中每个像素，计算其对应的投影射线的 Plücker 坐标 $(\mathbf{d}, \mathbf{m})$，作为 6 维的几何特征附加到 RGB token 上**。

```
像素 (u, v)
  → 相机系射线方向  d_cam  = normalize(K⁻¹ · [u, v, 1]ᵀ)
  → 基座系射线方向  d_base = R_base←cam · d_cam
  → 基座系光心      o_base = T_base←cam[:3, 3]
  → Plücker 坐标    (d_base,  o_base × d_base)   →  6 维向量
```

整张图像所有像素的 Plücker 坐标排列起来，就构成一张 $[H, W, 6]$ 的 Plücker ray map。

### 与本文 ray map 的关系

本文 §3.4 中的 ray map 用 $(\mathbf{r}_{\text{base}},\ \mathbf{o}_{\text{base}})$（方向 + 光心）表示，也是 6 维；Plücker 坐标用 $(\mathbf{d},\ \mathbf{m})$（方向 + 力矩）表示，同样 6 维。**两者数学上等价**，可以相互转换，编码的信息完全相同——都是"这个像素对应三维空间中的哪条射线"。实践中选哪种取决于与下游网络的兼容性。

### 量化收益

See like a Robot 的消融（§3.2.1）显示，**单独用 Plücker 射线的收益为 +0.8**（27.9 → 28.7），远小于 pointmap 的 +6.8（27.9 → 34.7）。原因是射线只告诉模型"从哪看、朝哪看"，而 pointmap 进一步告诉模型"看到的东西在 3D 空间的具体位置"。但两者互补——射线提供几何先验，pointmap 提供空间定位，本方案建议叠加使用（§3.4 (e)）。
