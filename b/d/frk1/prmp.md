...
回HOME, 重新开始实验. 参考 @RLmm/b/d/frk1/4debug/cursor_experiment.md 中记录的"实验A2", 将 `GRASP_HANDOFF_A` 设为 0.40, 将 n_exec 变回 5, 其它不变, 确保改动生效后, 启用 @RLmm/b/x/4dwvla_ext/franka_vla_client.py  , @RLmm/b/x/4dwvla_ext/vla_inference_server.py 和 /home/nvidia/bt/ckp/4wvlaFrk/plug/4wvlaFrkPlugCkp010420/ 这个checkpoint, 再跑一下实验. client 端要在容器 `rlinf-4dwvla-franky` 中执行, server 端在容器`rlinf-4dwvla-gpu`中执行, 把这次实验成为"实验A2_1_3".