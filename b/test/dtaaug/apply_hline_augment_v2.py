#!/usr/bin/env python3
"""CLI demo for :class:`VideoRandomHorizontalLinePerFrame` in ``augmentation.py``.

Each frame independently may have no line or a horizontal segment with random
row, color, thickness, and length.

Examples:
    python b/test/apply_hline_augment_v2.py --demo --output_dir b/test/hline_vis_v2
    python b/test/apply_hline_augment_v2.py --input clip.mp4 --output clip_hline.mp4
    python b/test/apply_hline_augment_v2.py --r1_sample --sample_idx 0
"""
...
VideoRandomHorizontalLinePerFrame = _aug.VideoRandomHorizontalLinePerFrame


def build_hline_transform(
    *,
    p_line: float = 0.7,
    ...
    p: float = 1.0,
) -> VideoRandomHorizontalLinePerFrame:
    """Build :class:`VideoRandomHorizontalLinePerFrame` (clip-level ``p`` via base class)."""
    return VideoRandomHorizontalLinePerFrame(
        p_line=p_line,
        line_width_range=line_width_range,
        length_frac_range=length_frac_range,
        row_range=row_range,
        color=color,
        p=p,
    )
...
def save_comparison(
    ...
    title: str = "Per-frame random horizontal lines",
) -> None:
    import matplotlib
    ...
    if frame_indices is None:
        frame_indices = list(range(min(9, original.shape[0])))
    ...
    for col, fi in enumerate(frame_indices):
        for row, tensor in enumerate([original, augmented]):
            ...
            label = "orig" if row == 0 else "aug"
            axes[row, col].set_title(f"{label} f{fi}", fontsize=8)

    fig.suptitle(
        f"{title}\n(VideoRandomHorizontalLinePerFrame: per-frame on/off, row, width, length)",
        fontsize=11,
    )
    fig.tight_layout()
...
def load_r1_robotwin_video(data_dir: str, fastwam_root: str, sample_idx: int) -> torch.Tensor:
    ...
    work_dir = os.path.join(os.path.dirname(__file__), "hline_vis_v2")
    ...
    top = TF.resize(cameras["head_rgb"], [256, 320], antialias=True)
    left = TF.resize(cameras["left_wrist_rgb"], [128, 160], antialias=True)
    right = TF.resize(cameras["right_wrist_rgb"], [128, 160], antialias=True)
    return torch.cat([top, torch.cat([left, right], dim=-1)], dim=-2)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Demo VideoRandomHorizontalLinePerFrame from rlinf fastwam augmentation"
    )
    parser.add_argument("--input", type=str, default=None)
    parser.add_argument("--output", type=str, default=None)
    parser.add_argument("--output_dir", type=str, default="b/test/hline_vis_v2")
    parser.add_argument("--demo", action="store_true")
    parser.add_argument("--r1_sample", action="store_true")
    parser.add_argument("--sample_idx", type=int, default=0)
    parser.add_argument("--data_dir", type=str, default=None)
    parser.add_argument("--p_line", type=float, default=0.7, help="Per-frame line probability")
    parser.add_argument(
        "--p",
        "--p_clip",
        dest="p",
        type=float,
        default=1.0,
        help="Clip-level apply probability (VideoAugmentation.p)",
    )
    parser.add_argument("--width_min", type=int, default=1)
    parser.add_argument("--width_max", type=int, default=4)
    parser.add_argument("--length_min", type=float, default=0.3)
    parser.add_argument("--length_max", type=float, default=1.0)
    ...
    augmented = apply_hline(
        video,
        p_line=args.p_line,
        line_width_range=(args.width_min, args.width_max),
        length_frac_range=(args.length_min, args.length_max),
    ...
    out_png = os.path.join(args.output_dir, "hline_comparison_v2.png")
